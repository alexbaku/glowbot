...
""",
    ...
    def _clean_message_history(self, raw_messages: List) -> List[Dict]:
        ...
        for idx, msg in enumerate(raw_messages):
            ...
            if isinstance(content, str):
                if not content.strip():
                    logger.warning(
                        f"Skipping message {idx}: empty or whitespace-only content"
                    )
                    continue
            ...
            # FILTER 4: Handle list content (multimodal)
                ...
                logger.warning(
                    f"Skipping message {idx}: unexpected content type {type(content)}"
                )
        ...
        logger.info(
            f"Cleaned message history: {len(raw_messages)} -> {len(final)} messages"
        )
...
"""
    ...
    async def _save_response(
        self, db: AsyncSession, user_id: int, message: str, context: ConversationContext
    ) -> None:
        """Save assistant message and updated context"""

        # Save message
        await self.conversation_repo.add_assistant_message(
            db=db, user_id=user_id, content=message
        )

        # Save context
        await self.conversation_repo.save_context(
            db=db, user_id=user_id, context=context
        )

    async def analyze_skin_image(
        self, image_url: str, user_context: ConversationContext = None
    ) -> dict:
        """
...
"""
            ...
            return {"success": True, "analysis": analysis_text, "image_url": image_url}
